...
export function evaluateOopsSubmission(input: unknown): EvaluationResult {
  ...
  const detectedOopsTags = uniqueList(passedCases.flatMap((item) => item.tags));
...
function inferRedFlags(scores: OopsScores, allowedRedFlags: string[]) {
  ...
  if (scores.abstraction_score < 40) inferred.push('no-shared-abstraction');
  ...
  if (scores.solid_principles_score < 40)
