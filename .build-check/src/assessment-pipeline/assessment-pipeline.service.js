...
exports.AssessmentPipelineService = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const supabase_js_1 = require("@supabase/supabase-js");
const ws_1 = __importDefault(require("ws"));
const dashboard_evaluation_service_1 = require("../evaluations/dashboard-evaluation.service");
const dsa_evaluation_service_1 = require("../evaluations/dsa-evaluation.service");
const mcq_evaluation_service_1 = require("../evaluations/mcq-evaluation.service");
const oops_evaluation_service_1 = require("../evaluations/oops-evaluation.service");
const sql_evaluation_service_1 = require("../evaluations/sql-evaluation.service");
const question_bank_service_1 = require("../question-bank/question-bank.service");
let AssessmentPipelineService = class AssessmentPipelineService {
    config;
    questionBank;
    dsaEvaluation;
    sqlEvaluation;
    ...
    dashboardEvaluation;
    supabase;
    constructor(config, questionBank, dsaEvaluation, sqlEvaluation, oopsEvaluation, mcqEvaluation, dashboardEvaluation) {
        this.config = config;
    ...
    async finalize(rawInput) {
        ...
        const bank = (await this.questionBank.getBank());
        const questions = bank.questions || [];
        const questionsById = new Map(questions.map((question) => [question.id, question]));
        const submittedAt = input.submitted_at || new Date().toISOString();
        const durationMinutes = input.duration_minutes || bank.assessment?.duration_minutes || 180;
        ...
        const dashboardEvaluation = await this.tryEvaluate('DASHBOARD', () => this.dashboardEvaluation.evaluate(dashboardInput), this.fallbackDashboard(input, bank, deterministicScores));
    ...
    parseInput(rawInput) {
        if (!rawInput || typeof rawInput !== 'object' || Array.isArray(rawInput)) {
            throw new common_1.BadRequestException('Finalize payload must be a JSON object');
        }
        const input = rawInput;
        if (!input.student_id)
            throw new common_1.BadRequestException('student_id is required');
        if (!input.answers || typeof input.answers !== 'object') {
            throw new common_1.BadRequestException('answers are required');
        }
        return input;
    }
    async evaluateCodingSection(section, questions, input) {
        ...
        for (const detail of details) {
            ...
            evaluations.push(await this.tryEvaluate(section, () => section === 'DSA'
                ? this.dsaEvaluation.evaluate(detail)
                : this.oopsEvaluation.evaluate(detail), fallback));
        }
        return {
            score: this.average(evaluations.map((item) => Number(item.output.overall_question_score || 0))),
            evaluations,
            deterministic: { questions: details },
        };
    }
    async evaluateSqlSection(questions, input) {
        ...
        for (const detail of details) {
            evaluations.push(await this.tryEvaluate('SQL', () => this.sqlEvaluation.evaluate(detail), this.fallbackQuestionEvaluation('SQL', detail)));
        }
        return {
            score: this.average(evaluations.map((item) => Number(item.output.overall_question_score || 0))),
            evaluations,
    ...
    async evaluateMcqSection(questions, input) {
        ...
        const evaluation = await this.tryEvaluate('MCQ', () => this.mcqEvaluation.evaluate(payload), {
            section: 'MCQ',
    ...
    async tryEvaluate(section, evaluate, fallback) {
        try {
            ...
            return {
                ...
                output: {
                    ...fallback.output,
                    evaluation_error: error instanceof Error ? error.message : `${section} evaluation failed`,
                },
            };
        }
    }
    async persistAttempt(input, bank, submittedAt, durationMinutes) {
        ...
        const { data, error } = await supabase
            ...
            .single();
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not create assessment attempt: ${error.message}`);
        }
        return String(data.id);
    }
    async persistQuestionAttempts(attemptId, input, questions) {
        ...
        const { error } = await this.getSupabase()
            ...
            .insert(rows);
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not persist question attempts: ${error.message}`);
        }
    }
    async persistEvaluations(attemptId, questionsById, evaluations) {
        ...
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not persist question evaluations: ${error.message}`);
        }
    }
    async persistFinalRuntimeSnapshots(attemptId, input, questions) {
        await Promise.all([
    ...
    async persistFinalCodeRuns(attemptId, input, questions) {
        ...
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not persist final code run snapshots: ${error.message}`);
        }
    }
    async persistFinalSqlRuns(attemptId, input, questions) {
        const rows = questions
        ...
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not persist final SQL run snapshots: ${error.message}`);
        }
    }
    async persistFinalMcqAnswers(attemptId, input, questions) {
        const rows = questions
        ...
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not persist final MCQ answers: ${error.message}`);
        }
    }
    parseTestSummary(message) {
    ...
    async persistReport(params) {
        const output = params.dashboardEvaluation.output;
        const sectionScores = {
        ...
        const row = {
            ...
            assessment_title: params.bank.assessment?.title || 'JoraIQ College Assessment',
        ...
        const { data, error } = await this.getSupabase()
            ...
            .single();
        if (error) {
            throw new common_1.InternalServerErrorException(`Could not write dashboard report: ${error.message}`);
        }
        return data;
    }
    fallbackQuestionEvaluation(section, detail) {
    ...
    fallbackDashboard(input, bank, scores) {
        ...
        const weighted = (scores.DSA * Number(weights.DSA || 0) +
        ...
        return {
            ...
            output: {
                ...
                company_recommendation: weighted >= 70
    ...
    getSupabase() {
        ...
        const serviceKey = this.config.get('SUPABASE_SERVICE_ROLE') ||
            this.config.get('SUPABASE_SERVICE_ROLE_KEY') ||
            this.config.get('SUPABASE_KEY');
        if (!supabaseUrl || !serviceKey) {
            throw new common_1.InternalServerErrorException('Supabase service credentials are not configured');
        }
        this.supabase = (0, supabase_js_1.createClient)(supabaseUrl, serviceKey, {
    ...
    computeReadiness(params) {
        const reasons = [];
        const concerns = [];
        const severeRisk = params.compilationBehaviour === 'Failed' ||
        ...
        if (params.hiddenTestPassRate < 30)
            reasons.push(`Hidden test pass rate is weak at ${params.hiddenTestPassRate}.`);
        ...
        else if ((params.marksScore >= 45 && params.marksScore <= 60) ||
    ...
    trainingPriority(weakestSection, sectionScores) {
        const score = sectionScores[weakestSection] ?? 0;
        if (weakestSection === 'DSA')
            return score < 45 ? 'Core problem solving and hidden-case handling' : 'Algorithmic optimization and edge-case practice';
        if (weakestSection === 'SQL')
            return score < 45 ? 'SQL correctness, joins, and result validation' : 'SQL efficiency, NULL handling, and business rules';
        if (weakestSection === 'OOPs')
            return score < 45 ? 'Object modelling, abstraction, and class design' : 'SOLID design, extensibility, and structure';
        return score < 45 ? 'Fundamental CS revision and accuracy drills' : 'MCQ revision for weaker theory areas';
    }
    teacherAction(bucket, trainingPriority, severeRisk) {
        if (bucket === 'Ready') {
...
exports.AssessmentPipelineService = AssessmentPipelineService;
exports.AssessmentPipelineService = AssessmentPipelineService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [config_1.ConfigService,
        question_bank_service_1.QuestionBankService,
        dsa_evaluation_service_1.DsaEvaluationService,
...
//# sourceMappingURL=assessment-pipeline.service.js.map